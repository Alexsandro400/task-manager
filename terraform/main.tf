...
resource "null_resource" "k3d_cluster" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      CMD="k3d cluster create $CLUSTER_NAME --servers 1 --agents $NODE_COUNT"
      if [ "${var.k3d_wait}" = "true" ]; then
        CMD="$CMD --wait"
      fi
      $CMD
      k3d image import task-manager:local -c $CLUSTER_NAME
...
resource "null_resource" "deploy_app" {
  ...
  provisioner "local-exec" {
    command = "kubectl --kubeconfig=${var.kubeconfig_path} apply -f ${path.module}/k8s/"
  }
